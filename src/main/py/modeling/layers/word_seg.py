...
sents = open('/Users/xueyu/Workspace/training data/icwb2-data/training/msr_training.utf8').read()
...
def prepare_data():
    ...
    X = [x + [0] * (maxlen - len(x)) for x in X]  # 不足则补零
    Y = [y + [4] * (maxlen - len(y)) for y in Y]  # 不足则补第五个标签
    return np.array(X), to_categorical(Y, 5)


from modeling.layers.crf_keras import CRF
...
tag_score = Dense(5)(embedding)  # 变成了5分类，第五个标签用来mask掉
...
model.save('seg-crf-keras-model.h5')

model.load_weights('seg-crf-keras-model.h5')
_ = model.get_weights()[-1][:4, :4]
trans = {}
for i in 'sbme':
    for j in 'sbme':
        trans[i + j] = _[tag2id[i], tag2id[j]]
